...
The GP model returns a Gaussian distribution (a mean and standard deviation) for its prediction at any given input. This accounts for measurement uncertainty in the data, as illustrated by the example above.



### How I use GPs in my research

I use GP regression in my [research](https://hematthi.github.io/research/syssim_architectures.html), to model simulated data (actually, to model how well the simulated models fit the real data, as explained below), as a way of more quickly constraining the parameters of the models. We use [approximate Bayesian computation (ABC)](https://en.wikipedia.org/wiki/Approximate_Bayesian_computation) to constrain the model parameters. This involves specifying a distance function (which quantifies how well the models fit the data) and trying to minimize that function.

While we could perform model inference with ABC without any GP regression, our models are relatively expensive and have many parameters. It would take too long to simulate for millions of different sets of model parameters. Thus, we use GP's to *model the distance function itself*. Since the distance is a function of the model parameters, the "data" in this case is a set of model parameters with known distances (i.e., where we actually simulated the model and compared it to the data). In other words, we use GP's to build an **emulator** for our models.

This problem is especially difficult because unlike the examples above where the function is of a single variable, our function is of multiple variables (the distance is a function of around a dozen model parameters)! Indeed, GP regression can be used to model multivariate functions. However, the larger parameter space means that more training points are needed to model the behaviour of the function. Unfortunately, we are limited in how many training points we can use due to the complexity of computing the GP model (e.g. inverting a matrix). Thus, we choose our training points in the vicinity of the minimum region (found using other methods, such as an optimization scheme), and set a constant mean function that is well above the minimum distances so that predictions in regions far from any training data will never return better distances than predictions near the minimum. For the kernel function, we use the SE kernel, generalized to multiple variables (the term in the exponential becomes a sum over differences in each variable/dimension, with a separate length scale for each dimension).
